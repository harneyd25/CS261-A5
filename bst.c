...
int bst_size(struct bst* bst) {
  ...
  struct bst bst_left = { bst->root->left };
  struct bst bst_right = { bst->root->right };

  // recursive calls to left and right children
  return 1 + bst_size(&bst_left) + bst_size(&bst_right);
}
...
int bst_height(struct bst* bst) {
  ...
  // getting heights of subtrees
  int left_height = bst_height(&bst_left);
  int right_height = bst_height(&bst_right);

  // returning 1 + max(left subtree height, right subtree height)
 ...
 *
 * Works by subtracting the current nodes value from sum
 * and recursively calling on left and right subtrees
 */
int bst_path_sum(int sum, struct bst* bst) {
  assert(bst);

  // base case: reached the end of a path (either empty root, or reached leaf)
  ...
  // subtracting current node value from sum
  sum -= bst->root->val;

  // recursively calling on subtrees
  return (bst_path_sum(sum, &bst_left) || bst_path_sum(sum, &bst_right));
}
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
  ...
  // initialize the bst_iterator_struct
  struct bst_iterator* bst_iter = malloc(sizeof(struct bst_iterator));
  assert(bst_iter);
  bst_iter->s = stack_create();
  bst_iter = bst;

  // pushing the valus from the bst onto the stack in reverse in-order
    struct bst_node* current = bst->root;
    // reach
	while (current != NULL) {                // loops through entire bst
      stack_push(bst_iter->s, current->val);
